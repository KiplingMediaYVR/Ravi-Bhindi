   ...
   */
	...
	public function SearchQuery($resource, $class, $query = "", $optional_params = array()) {
		...
		while ($continue_searching) {
			...
			$xml = simplexml_load_string($body, "SimpleXMLElement", 0, "urn:CREA.Search." . $resource, false);
			$return = array();
					...
					if($k === "RETS-RESPONSE")
					{
						foreach($xml->{'RETS-RESPONSE'}->children() as $k => $v)
							...
							if($k === "PropertyDetails" || $k === "Property")
								$return []= json_decode(json_encode((array)$v),1);

							if($k === "AgentDetails" || $k === "Agent")
								$return []= json_decode(json_encode((array)$v),1);

							if($k === "OfficeDetails" || $k === "Office")
								$return []= json_decode(json_encode((array)$v),1);
			...
			$key = ( $resource == 'Property' ) ? 'Properties' : $resource ;

			$this->FreeResult($this->int_result_pointer);
			return array("Count" => $count, $key => $return);
		}
	}
	...
	public function Disconnect() {
		...
		if (empty($this->capability_url['Logout'])) {
			error_log("Disconnect() called but unable to find Logout location.  Failed login?");
		...
		error_log("## Disconnected from RETS ...");
		error_log("=============================================");
	...
	public function ParseXMLResponse($data = "")
	{
		...
		if (!empty($data))
		{
			...
			$data = str_replace("xmlns=\"urn:CREA.Search.Agent\"", "xmlns=\"\"", $data);
			$data = str_replace("xmlns=\"urn:CREA.Search.Office\"", "xmlns=\"\"", $data);
