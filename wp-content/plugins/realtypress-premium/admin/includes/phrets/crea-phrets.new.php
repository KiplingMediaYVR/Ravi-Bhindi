...
class CREA_phRETS {
   ...
   */
	...
	public function GetObject($resource, $type, $id, $photo_number = '*', $location = 0) {
		...
		echo "<p>------------------------------------------------------------</p>";
		echo "<p>Start GetObject() " . (memory_get_usage(false)/1024/1024) . "</p>";
		...
		if( strpos($photo_number, ',') !== false ){
			...
			$photo_number = str_replace(',', ':', $photo_number);
		}
		...
		if( strpos($photo_number, ':') !== false ){
			...
			$requested_numbers = null;
			// $send_numb = preg_replace('/\:$/', '', $send_numb);
			$send_numb = str_replace(':', '', $send_numb);
		}
		else {
		...
		if( strpos($id, ',') !== false ){
			...
			$send_numb = str_replace(':', '', $send_id);
		}
		else {
			$send_id = trim($id).':'.$send_numb;
		}

		echo "<p>Before RETSRequest(): " . (memory_get_usage(false)/1024/1024) . "</p>";
		echo "<br>" . $this->capability_url['GetObject'];
		echo "<br>" . pp(array('Resource' => $resource, 'Type' => $type, 'ID' => $send_id, 'Location' => $location));
		// make request
		...
		// pp(count($result));
		echo "<p>After RETSRequest(): " . (memory_get_usage(false)/1024/1024) . "</p>";
		...
		echo "<p>BLA preg_match(): " . (memory_get_usage(false)/1024/1024) . "</p>";		

		// pp( $this->last_response_headers['Content-Type']);
		...
		if( strpos( $this->last_response_headers['Content-Type'], 'multipart/') !== false ){
			...
			echo "<p>Before preg_match(): " . (memory_get_usage(false)/1024/1024) . "</p>";
			...
			// 	$boundary = $matches[1];
			// }
			$boundary = "creaboundary";

			echo "<p>After preg_match(): " . (memory_get_usage(false)/1024/1024) . "</p>";

			echo "<p>Before preg_replace(): " . (memory_get_usage(false)/1024/1024) . "</p>";
			...
			echo "<p>After preg_replace(): " . (memory_get_usage(false)/1024/1024) . "</p>";
			...
			pp(count( $multi_parts ));


			echo "<p>Before array_pop, array_shift(): " . (memory_get_usage(false)/1024/1024) . "</p>";
			...
			echo "<p>After array_pop, array_shift(): " . (memory_get_usage(false)/1024/1024) . "</p>";
			...
			foreach ($multi_parts as $part) {
				...
				echo "<p>---------------------</p>";
				echo "<p>Start Multipart Loop: " . (memory_get_usage(false)/1024/1024) . "</p>";
				...
				foreach ($body_parts as $line) {
					...
					echo "<p>-->foreach(body_parts): " . (memory_get_usage(false)/1024/1024) . "</p>";
				...
				echo "<p>End Multipart Loop: " . (memory_get_usage(false)/1024/1024) . "</p>";
				echo "<p>---------------------</p>";
			...
			echo "<p>End GetObject() " . (memory_get_usage(false)/1024/1024) . "</p>";
			echo "<p>============================================================</p>";
		}
		else {
